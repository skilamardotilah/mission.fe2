...
export default function SeriesPlayer({ onClose }) {
  const [activePopup, setActivePopup] = useState(null);
  const [isPlaying, setIsPlaying] = useState(false);
  const [currentSpeed, setCurrentSpeed] = useState(1);
  const [isFullscreen, setIsFullscreen] = useState(false);
  const [currentEpisode, setCurrentEpisode] = useState(2); // Episode 2 is currently playing
  const videoRef = useRef(null);

  const episodes = [
    { id: 1, title: 'Pilot', description: 'American football coach Ted Lasso is hired by a wealthy divorcee to coach the English soccer team AFC Richmond' },
    { id: 2, title: 'Biscuits', description: "It's Ted's first day of coaching, and fans aren't happy. He makes little headway but remains undeterred as the team..." },
    { id: 3, title: 'Trent Crimm: Independent', description: 'To arrange an in-depth exposé, Rebecca pairs cynical journalist Trent Crimm with Ted for a day. Ted and Roy...' },
    { id: 4, title: 'For The Children', description: "Rebecca hosts the team's annual charity benefit, where Ted stages a reconciliation between Roy and Jamie." },
    { id: 5, title: 'Tan Lines', description: 'With his wife and son visiting from America, Ted makes drastic changes to the lineup during a critical match.' }
  ];
  ...
  useEffect(() => {
    const handleClickOutside = (e) => {
      ...
      const isPopup = e.target.closest('[id$="Popup"]') || e.target.closest('[id$="Menu"]') || e.target.closest('[id$="Episode"]') || e.target.closest('[id$="List"]') || e.target.closest('[id$="terjemahan"]');
  ...
  return (
    <div className="bg-[#181A1C] text-white font-lato relative w-full h-screen overflow-hidden">
      ...
      <div className="relative w-full h-screen bg-black overflow-hidden">
        <img src={asset('img/series/popup.png')} alt="Background" className="w-full h-full object-cover" />
        ...
        <div className="absolute bottom-0 w-full px-6 py-4 bg-black/60 text-white">
          <div className="flex items-center justify-between">
            ...
            <div className="text-base font-medium text-gray-200">
              TED Lasso Episode {currentEpisode}: {episodes.find(ep => ep.id === currentEpisode)?.title}
            </div>
            ...
            <div className="flex items-center gap-4 relative text-white justify-center">
              ...
              {/* Episode List Button */}
              <button onClick={() => togglePopup('episodeList')} title="Daftar Episode" className="hover:scale-110 transition">
              ...
              {activePopup === 'nextEpisode' && (
                <div className="absolute bottom-12 right-12 bg-[#2A2C2E] text-white rounded-md text-sm shadow-lg w-[440px] h-[160px] z-50 overflow-hidden">
                  ...
                  <div className="flex items-center p-3 gap-3 mt-4">
                    ...
                    <div>
                      <div className="text-white font-bold">Episode 3: Trent Crimm: Independent</div>
              ...
              {/* Episode List Popup */}
              {activePopup === 'episodeList' && (
                <div className="absolute bottom-12 right-12 bg-gray-700 text-white rounded-md text-sm shadow-lg w-[440px] h-[270px] z-50 overflow-hidden">
                      ...
                      >
                        {episode.id === currentEpisode ? (
                          <div className="flex gap-3">
                            <img src={asset('img/Poster/3.png')} alt={`Ep ${episode.id}`} className="w-[128px] h-[72px] object-cover rounded-md" />
                            <div>
